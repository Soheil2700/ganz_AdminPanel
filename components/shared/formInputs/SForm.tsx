...
import SwitchBox from "./formInputs/SwitchBox";
import FileInput from "./formInputs/FileInput";
import DateInput from "./formInputs/DateInput";
import PasswordInput from "./formInputs/PasswordInput";
import SliderInput from "./formInputs/SliderInput";
import UploadButton from "./formInputs/UploadButton";
import Gender from "./formInputs/Gender";
...
const SForm = ({
  ...
  return (
    ...
    >
      <div className={`${formClassName} grid lg:grid-cols-12 sm:grid-cols-2 grid-flow-row gap-4 justify-between items-start mt-5`}>
        {formStructure.map((item: any, index) => {
          switch (item.type) {
            ...
            case "textarea":
              return (
                <div className={item.col ? colObj[item.col] : "col-span-12"}>
                  <TextField key={index} id={index} {...item} multiline={true} rows={3} control={control} error={errors[item.name]} />
                </div>
              );
            case "file":
              return (
            ...
            case "gender":
              return (
                <div className={item.col ? colObj[item.col] : "col-span-3"}>
                  <Gender key={index} id={index} {...item} control={control} error={errors[item.name]} setValue={setValue} />
                </div>
              );
            case "component":
